...
xepOnline.Formatter = {
    ...
    replaceCanvas: function (dest) {
        jQuery(dest).find('canvas').each(function (index) {
            var canvas = this;
            var sr_canvas = jQuery(jQuery(xepOnline.Formatter.__elm)[0]).find('canvas')[index];
            jQuery('<img src="' + sr_canvas.toDataURL() + '"/>').insertAfter(canvas);
        });
    },
    ...
    getFormData: function (PrintCopy, Name, MimeType, FileName, Resolution) {
        ...
        var blob;
        try {
            blob = new Blob([data], { type: xepOnline.Formatter.sr_type.xml });
        }
        catch (e) {
            if (e.name === 'TypeError') {
    ...
    cleanSVGDeclarations: function (data) {
        ...
        var startIdx = 0;
        var svgde_text = 'xmlns="http://www.w3.org/2000/svg"';
        ...
        while (match != null) {
            ...
            var svgde_flag = false;
            var namevalues = match[0].match(/([^< =,]*)=("[^"]*"|[^,"]*)/ig);
            for (var s = 0; s < namevalues.length; s++) {
                if (namevalues[s] === svgde_text && svgde_flag) { } else {
                    builder += ' ' + namevalues[s];
                }
                svgde_flag = namevalues[s] === svgde_text || svgde_flag;
            }
            builder += '>';
